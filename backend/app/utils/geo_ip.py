...
async def get_location_from_ip(ip_address: str) -> dict:
    ...
    try:
        ...
        span.end(status_message=f"Request Error: {e}", level="ERROR")
    except httpx.HTTPStatusError as e:
        print(f"[ERROR] Geoapify API returned an error status {e.response.status_code}: {e.response.text}")
        span.end(status_message=f"HTTP Error: {e.response.status_code}", level="ERROR")
    except Exception as e:
        print(f"[ERROR] An unexpected error occurred: {e}")
        span.end(status_message=f"Unexpected Error: {e}", level="ERROR")
